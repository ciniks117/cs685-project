...
for coord in LAT_LON_Dict:
    ...
    if sent>0 and sent<=0.25:
        ...
        folium.Circle( location=[ coord[0], coord[1] ], color='green', fill_color='green', radius=50*len(LAT_LON_Dict[coord]) ).add_to( mapit )
...
for coord in LAT_LON_Dict:
    ...
    if (ind_coord.latitude,ind_coord.longitude) == coord:
        ...
        continue
    if sent>0 and sent<=0.25:
        folium.Circle( location=[ coord[0], coord[1] ], color='darkred', fill_color='darkred', radius=30000).add_to( mapit )
    elif sent>0.25 and sent<=0.5:
        folium.Circle( location=[ coord[0], coord[1] ], color='red', fill_color='red', radius=30000 ).add_to( mapit )
    elif sent>0.5 and sent<=0.75:
        folium.Circle( location=[ coord[0], coord[1] ], color='green', fill_color='green', radius=30000 ).add_to( mapit )
    elif sent>0.75 and sent<=1:
        folium.Circle( location=[ coord[0], coord[1] ], color='darkgreen', fill_color='darkgreen', radius=30000).add_to( mapit )

mapit.save( 'indiamap_simple_farmer.html')
...
mapit.save( 'worldmap_farmer.html')
